...
public class SmisUtils {
	...
	public static final String[] PROPOSALS = new String[] { "CH", "ES", "EV", "HC", "HG", "LS", "MA", "ME", "MI", "MX", "SC", "MD" };
	public static final Set<String> PROPOSAL_SET = new HashSet<String>(Arrays.asList(PROPOSALS));
	...
	static Long pushSmisMetadataToIcat(String proposalName, String beamline,
		...
		try {
			...
			logger.error("Impossible retrieve proposal <"+proposalCode+", "+ proposalNumber+"> from SMIS." + e.getMessage());
			throw e;
		}
		try{
			// If does not exist it rise an exceprion.
			stub.findSessionByProposalBeamlineAndDate(proposalCode,	proposalNumber, beamline, toCalendar(date));
		}catch (org.jboss.axis.AxisFault e) {
			logger.error("Impossible retrieve Session <"+proposalCode+", "+ proposalNumber+"> from SMIS." + e.getMessage());
			throw e;
		}

		if (proposalPk > 0) {// Proposal exist in SMIS?
			logger.info("Proposal '"+proposalCode+"' exists in SMIS. ");
			...
			if (!IcatUtils.existStudy(proposalName)) {
				logger.info("Study '"+proposalCode+"' do NOT exists in ICAT: Creating.... ");
				...
				//System.out.println(MISBeanUtils.beanToString(mainProposer)); // DEBUG
				...
				logger.info("Created GROUP.. "+MISBeanUtils.beanToString(group));
				
				// Create proposal USER in Icat
				// -----------------------------------------------------------
				createIcatUser(icat, sessionId, group, proposalName, proposalName);

				
				// Create main proposal USER in Icat
				// -----------------------------------------------------------
				createIcatUser(icat, sessionId, group, mainProposer.getScientistName(), mainProposer.getScientistFirstName());
				...
				study.setStatus(StudyStatus.IN_PROGRESS); // how to set it to FINISH?

				study.setCreateTime(toXmlGregorian(getCurrentTime())); // should be automatic
				study.setDescription(mainProposer.getProposalTitle());

				User mainUser = IcatUtils.getUser(
						mainProposer.getScientistFirstName(),
						mainProposer.getScientistName());
				...
				logger.info("Created STUDY.. "+MISBeanUtils.beanToString(study));

			}// --Created new STUDY in Icat--
			else{
				logger.info("Study '"+proposalCode+"' already exists in ICAT. ");
			}
			...
			try {
				expSession = stub
						.findSessionByProposalBeamlineAndDate(proposalCode,	proposalNumber, beamline, cal);
				//expSession = expSessions[0];
			} catch (Exception e) {

				Exception ex = new IOException("Impossible push SMIS data into ICAT for <Proposal, beamline, date>: <"
			...
			//System.out.println(MISBeanUtils.beanToString(expSession)); // DEBUG
			
			if (expSession != null) { // Investigation exist in SMIS? 
				//logger.info("Investigation <"+ proposalName + ", " + beamline + ", " + date+ "> exists in SMIS. ");
				
				String invName = formatInvestigationName(expSession.getName(), proposalCode, proposalNumber, beamline);
				...
				if (tmp == null) {
					//logger.info("Investigation '"+ invName+ "' do NOT exists in ICAT: Creating.... ");
					...
					for (ProposalParticipantInfoLightVO usr : users) {
						...
						// -----------------------------------------------------------
						createIcatUser(icat, sessionId, proposalName, usr.getScientistName(), usr.getScientistFirstName());
					}
					...
					if (fc == null) {
						...
						fc.setStartDate(toXmlGregorian(expSession.getRunEndDate()));
						fc.setEndDate(toXmlGregorian(expSession.getRunStartDate()));
						...
						logger.info("Created FACILITYCYCLE.. "+MISBeanUtils.beanToString(fc));
					}
					...
					Instrument ins = IcatUtils.getInstrument(expSession.getBeamlineName());
					inv.setInstrument(ins);

					inv.setFacility(IcatUtils.getFacility("ESRF"));
					inv.setFacilityCycle(IcatUtils.getFacilityCycle(expSession.getRunCode()));
					...
					logger.info("Created INVESTIGATION.. "+MISBeanUtils.beanToString(inv));
					...
					logger.info("Created STUDYINVESTIGATION.. "+MISBeanUtils.beanToString(stInv));

					ProposalParticipantInfoLightVO[] sessionUsers = stub.findUsersByExpSession(expSession.getPk());
					for (ProposalParticipantInfoLightVO usr : sessionUsers) {
						//System.out.println(MISBeanUtils.beanToString(usr)); // DEBUG
						
						// Create  USER in Icat
						// -----------------------------------------------------------
						User icatUser = null;
						if (!IcatUtils.userExistInICAT(
								usr.getScientistFirstName(),
								usr.getScientistName())) {
							
							icatUser = createIcatUser(icat, sessionId, proposalName, usr.getScientistName(), usr.getScientistFirstName());
							
						}
						InvestigationUser invUser = new InvestigationUser();
						
						if(icatUser==null){
							icatUser = IcatUtils.getUser(usr.getScientistFirstName(), usr.getScientistName());
						}
						if(inv.getId()==null){
							inv.setId( IcatUtils.getInvestigation(inv.getName()).getId() );
						}
						
						// Create INVESTIGATIONUSER in Icat
						// -----------------------------------------------------------
						if(!IcatUtils.existInvestigationUser(icatUser.getId(), inv.getId())){
							
							invUser.setUser(icatUser);
							//	invUser.setRole(value) // Missing
							invUser.setInvestigation(inv);
							invUser.setRole("Scientist"); // 4.2.2 has a bug -> need this value!!
							//icatObjects.add(invUser);
							Long invUserID = icat.create(sessionId, invUser);
							invUser.setId(invUserID);
							
							logger.info("Created INVESTIGATIONUSER.. "+MISBeanUtils.beanToString(invUser));
						}	
					}
					
				}
				else{// if Investigation already exist in Icat
					logger.info("Investigation '"+ invName+ "' already exists in ICAT. ");
					invId = tmp.getId();
				}		
		...
		/*	SampleSheetInfoLightVO[] sampleSheet = stub
					.findSamplesheetInfoLightForProposalPk(proposalPk);
			System.out
					.println("----------------------------------------------------------------");
			...
			if(sampleSheet.length>0){
				...
				// Enter all study Samples
				for (SampleSheetInfoLightVO vo : sampleSheet) {
					System.out.println(MISBeanUtils.beanToString(vo));		
					
					TypeDesc tmp = vo.getTypeDesc();
					FieldDesc[] tmp2 = tmp.getFields();
					for(FieldDesc t: tmp2){	
						System.out.println("-- "+t.getFieldName()+" : "+MISBeanUtils.beanToString(tmp.getFieldByName(t.getFieldName())) );
					}
				}
			}*/
			...
			logger.info("Proposal '"+proposalCode+"' do NOT exists in SMIS. ");
		}
	 ...
	 * @throws IOException 
	 */
	private static User createIcatUser(ICAT icat, String sessionId, String groupName, String scientistName,
			String scientistFirstName) throws IcatException_Exception, IOException {
		
		Group group = IcatUtils.getGroup(groupName);
		return createIcatUser(icat, sessionId, group, scientistName, scientistFirstName);
	 ...
	 * @throws IOException 
	 */
	private static User createIcatUser(ICAT icat, String sessionId, Group group, String scientistName,
			String scientistFirstName) throws IcatException_Exception, IOException {
		...
		if (!IcatUtils.userExistInICAT(scientistFirstName,scientistName)) {
			...
			logger.info("Created USER.. "+MISBeanUtils.beanToString(icatUser));
		}
		...
		if(!IcatUtils.existUserGroup(scientistName, group.getName())){
			...
			logger.info("Created USERGROUP.. "+MISBeanUtils.beanToString(userGroup));
		}
		return icatUser;
	}
	 ...
	 * @return
	 */
	public static String formatInvestigationName(String proposalName, String proposalCode, Integer proposalNumber, String beamline) {
			
		String dates = proposalName.substring(proposalName.trim().lastIndexOf(" ")).trim();
				
		String prefix = proposalCode+proposalNumber.toString()+" "+beamline+" "+dates ;
		
		return prefix;
	}

	/**
	 * Extract the Proposal number from the proposal name and check if it is a number.
	 * Remove eventual leading "/" or "-" characters.
	 * @param proposalName2
	 * @return
	 */
	...
	/**
	 * Extract the Proposal code from the proposal name and check if exist in the list of allowed values.
	 * @param proposalName
	 * @return
	 * @throws Exception
	 */
	private static String extractProposalCode(String proposalName) throws Exception {
		// should probably check if the code exist in the Icat INVESTIGATIONTYPE table
		String code = proposalName.substring(0, 2).trim().toUpperCase();
		/*if(!PROPOSAL_SET.contains(code))
			throw new Exception("Proposal code not recognized. Received: '"+ code+"'. Expected: "+PROPOSAL_SET);*/
		return code;
	}
	...
	/**
	 * From a string of the form "2013/02/07T17:44:10" create 
	 * a Calendar object.
	 * @param val
	 * @return
	 */
